...
struct node *copy(struct node *head) {
    ...
    while (head != NULL) {
        ...
        // not general case -> empty list, i.e. new list
        // first node
        if (new_list == NULL) {
            new_list = new_node;
            end_new_list = new_node;
        }
        
        // general case:
        else {
            end_new_list->next = new_node;
            end_new_list = new_node;
            // end_new_list = end_new_list->next;
        }        
        head = head->next;
    }    
...
int identical(struct node *head1, struct node *head2) {
    ...
    // what has the while loop missed?
    // maybe the lists are different lengths??
